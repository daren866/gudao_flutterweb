...
// ====================================================================== 
// JS 运行时管理器 (Android / Windows / iOS / macOS 通用版) 
// ====================================================================== 
class JsRuntimeManager {
  ...
  static JavascriptRuntime _createRuntime() {
    ...
    js.evaluate(r'''
      ...
      var setInterval = function(fn) { if(typeof fn==='function') fn(); return 0; };
...
class _MyHomePageState extends State<MyHomePage> { 
  ...
  void _execScriptsFromHtml(String html) { 
    try { 
      ...
      for (final script in doc.getElementsByTagName('script')) { 
        ...
        if (type != null && type != 'text/javascript' && type != 'application/javascript' && type != '') continue; 
  ...
  Future<void> _submitForm(_FormData formData) async { 
    ...
    try { 
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('提交出错: $e'))); 
    } finally { 
      if (mounted) setState(() => _isLoading = false); 
    } 
  } 
  ...
  Widget? _buildHtmlInput(dom.Element element) { 
    ...
    return Padding(padding: const EdgeInsets.symmetric(vertical: 8.0), child: _InputWrapper( 
      type: type, placeholder: element.attributes['placeholder'] ?? '', value: element.attributes['value'] ?? '', 
      name: element.attributes['name'] ?? '', formData: formData, 
      onSubmit: formData != null ? () => _submitForm(formData) : null, onclick: element.attributes['onclick'], 
    )); 
  } 

  Widget? _buildHtmlButton(dom.Element element) { 
    final formData = _getFormDataForElement(element); 
    return Padding(padding: const EdgeInsets.symmetric(vertical: 8.0), child: _FormButtonWrapper( 
      type: element.attributes['type']?.toLowerCase() ?? 'button', text: element.innerHtml, formData: formData, 
      onSubmit: formData != null ? () => _submitForm(formData) : null, onclick: element.attributes['onclick'], 
    )); 
  } 
  ...
  Widget? _buildHtmlSelect(dom.Element element) { 
    ...
    return Padding(padding: const EdgeInsets.symmetric(vertical: 8.0), child: _SelectWrapper(items: items, name: element.attributes['name'] ?? '', formData: formData, onchange: element.attributes['onchange'])); 
  } 

  Widget? _buildHtmlTextarea(dom.Element element) { 
    final formData = _getFormDataForElement(element); 
    return Padding(padding: const EdgeInsets.symmetric(vertical: 8.0), child: _TextareaWrapper(name: element.attributes['name'] ?? '', placeholder: element.attributes['placeholder'] ?? '', value: element.text.trim(), formData: formData, oninput: element.attributes['oninput'])); 
  } 
  ...
  Widget? _buildHtmlScript(dom.Element element) { 
    ...
    if (src != null && content.isEmpty) { 
      return Container(margin: const EdgeInsets.symmetric(vertical: 8), padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.grey[100], borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.orange[300]!)), child: Row(children: [Icon(Icons.javascript, color: Colors.orange[700], size: 20), const SizedBox(width: 8), Expanded(child: Text('外部脚本: $src', style: TextStyle(color: Colors.orange[700], fontSize: 12)))])); 
    } 
    if (content.isNotEmpty) return _ScriptBlock(scriptContent: content, type: element.attributes['type'], onExecute: () => _execScript(content)); 
    return const SizedBox.shrink(); 
  } 

  Color _getColor(String l) => l == 'error' ? Colors.red[400]! : l == 'warn' ? Colors.yellow[400]! : l == 'info' ? Colors.blue[400]! : Colors.white70; 

  @override 
  Widget build(BuildContext context) { 
    return Scaffold( 
      appBar: AppBar(title: Text(widget.title), backgroundColor: Theme.of(context).colorScheme.inversePrimary, actions: [ 
        IconButton(icon: Badge(isLabelVisible: _consoleLogs.isNotEmpty, label: Text(_consoleLogs.length.toString()), child: const Icon(Icons.terminal)), onPressed: () => setState(() => _showConsole = !_showConsole)) 
      ]), 
      body: Column(children: [ 
        Padding(padding: const EdgeInsets.all(8.0), child: Row(children: [ 
          Expanded(child: TextField(controller: _urlController, onSubmitted: _fetchWebContent, decoration: InputDecoration(hintText: '输入网址', border: const OutlineInputBorder(), filled: true, fillColor: Colors.grey[200], prefixIcon: const Icon(Icons.language, size: 20)))), 
          const SizedBox(width: 8), 
          ElevatedButton(onPressed: _isLoading ? null : () => _fetchWebContent(_urlController.text), child: _isLoading ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : const Text('访问')) 
        ])), 
        ...
        Expanded(child: Container(decoration: BoxDecoration(border: Border.all(width: 2, color: Theme.of(context).dividerColor)), child: SingleChildScrollView(padding: const EdgeInsets.all(16.0), child: HtmlWidget(_htmlContent, onTapUrl: (url) { _fetchWebContent(url); return true; }, customWidgetBuilder: (e) { final t = e.localName; if (t == 'input') return _buildHtmlInput(e); if (t == 'button') return _buildHtmlButton(e); if (t == 'select') return _buildHtmlSelect(e); if (t == 'textarea') return _buildHtmlTextarea(e); if (t == 'option') return const SizedBox.shrink(); if (t == 'script') return _buildHtmlScript(e); return null; }, textStyle: const TextStyle(fontSize: 16), customStylesBuilder: (e) => {'margin': '0', 'padding': '0'})))), 
        if (_showConsole) Container(height: 200, color: const Color(0xFF1E1E1E), child: Column(children: [ 
          Container(padding: const EdgeInsets.all(8), color: const Color(0xFF2D2D2D), child: Row(children: [ 
          ...
          Expanded(child: ListView.builder(padding: const EdgeInsets.all(8), itemCount: _consoleLogs.length, itemBuilder: (c, i) { final log = _consoleLogs[i]; return Padding(padding: const EdgeInsets.symmetric(vertical: 2), child: Text('[${log['time']}] ${log['message']}', style: TextStyle(color: _getColor(log['level']!), fontSize: 11, fontFamily: 'monospace'))); })) 
        ])), 
        SafeArea(child: Container(width: double.infinity, padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8), color: Colors.grey[300], child: Text('当前 URL: $_currentUrl', style: const TextStyle(fontSize: 12, color: Colors.black54), overflow: TextOverflow.ellipsis))) 
      ]), 
    ); 
  } 
} 
...
class _ScriptBlockState extends State<_ScriptBlock> { 
  ...
  @override 
  Widget build(BuildContext context) { 
    return Container(margin: const EdgeInsets.symmetric(vertical: 8), decoration: BoxDecoration(color: const Color(0xFF282C34), borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.orange[300]!)), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [ 
      InkWell(onTap: () => setState(() => _exp = !_exp), child: Container(padding: const EdgeInsets.all(8), color: const Color(0xFF21252B), child: Row(children: [Icon(Icons.javascript, color: Colors.orange[400], size: 18), const SizedBox(width: 8), Text(widget.type ?? 'text/javascript', style: TextStyle(color: Colors.grey[400]!, fontSize: 11)), const Spacer(), Text('${widget.scriptContent.split('\n').length} 行', style: TextStyle(color: Colors.grey[500]!, fontSize: 10))]))) 
      , if (_exp) Container(padding: const EdgeInsets.all(12), child: Text(widget.scriptContent, style: TextStyle(color: Colors.green[300]!, fontSize: 12, fontFamily: 'monospace'))) 
      , if (_exp) Container(padding: const EdgeInsets.all(8), color: const Color(0xFF21252B), child: Row(mainAxisAlignment: MainAxisAlignment.end, children: [ 
          TextButton.icon(onPressed: widget.onExecute, icon: const Icon(Icons.play_arrow, size: 16), label: const Text('执行'), style: TextButton.styleFrom(foregroundColor: Colors.green[400])) 
        ])) 
    ])); 
  } 
} 
...
class _InputWrapperState extends State<_InputWrapper> { 
  ...
  Widget build(BuildContext context) { 
    if (['text','search','email','password','url','tel','number','date'].contains(widget.type)) 
      return TextField(controller: _c, obscureText: widget.type=='password', decoration: InputDecoration(hintText: widget.placeholder, contentPadding: const EdgeInsets.symmetric(horizontal: 8, vertical: 12), border: OutlineInputBorder(borderRadius: BorderRadius.circular(4)))); 
    if (widget.type == 'hidden') 
      return const SizedBox.shrink(); 
    if (widget.type == 'checkbox') 
      return Row(mainAxisSize: MainAxisSize.min, children: [ 
        Checkbox(value: _chk, onChanged: (v) { 
    ...
    if (widget.type == 'radio') 
      return GestureDetector(onTap: () { 
        ...
        Container(width: 20, height: 20, decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: _chk?Colors.blue:Colors.grey, width: 2)), child: _chk ? Center(child: Container(width: 10, height: 10, decoration: const BoxDecoration(shape: BoxShape.circle, color: Colors.blue))) : null), 
        const SizedBox(width: 8), 
        Text(widget.placeholder.isNotEmpty?widget.placeholder:widget.value) 
      ])); 
    if (widget.type == 'submit') 
      return ElevatedButton(onPressed: () { _click(); widget.onSubmit?.call(); }, style: ElevatedButton.styleFrom(backgroundColor: Theme.of(context).primaryColor, foregroundColor: Colors.white), child: Text(widget.value.isNotEmpty?widget.value:'Submit')); 
    if (widget.type == 'reset') 
      return ElevatedButton(onPressed: () { widget.formData?.values.clear(); _c.clear(); setState(()=>_chk=false); _click(); }, style: ElevatedButton.styleFrom(backgroundColor: Colors.grey, foregroundColor: Colors.white), child: Text(widget.value.isNotEmpty?widget.value:'Reset')); 
    return ElevatedButton(onPressed: _click, style: ElevatedButton.styleFrom(backgroundColor: Colors.white, foregroundColor: Colors.black87, side: const BorderSide(color: Colors.grey)), child: Text(widget.value.isNotEmpty?widget.value:widget.type.toUpperCase())); 
  } 
} 
...
class _FormButtonWrapper extends StatelessWidget { 
  ...
  Widget build(BuildContext context) { 
    ...
    if (type=='submit') 
      return ElevatedButton(onPressed: () { click(); onSubmit?.call(); }, style: ElevatedButton.styleFrom(backgroundColor: Theme.of(context).primaryColor, foregroundColor: Colors.white), child: Text(text.isEmpty?'Submit':text)); 
    if (type=='reset') 
      return ElevatedButton(onPressed: () { formData?.values.clear(); click(); }, style: ElevatedButton.styleFrom(backgroundColor: Colors.grey, foregroundColor: Colors.white), child: Text(text.isEmpty?'Reset':text)); 
    return ElevatedButton(onPressed: click, style: ElevatedButton.styleFrom(backgroundColor: Colors.white, foregroundColor: Colors.black87, side: const BorderSide(color: Colors.grey)), child: Text(text.isEmpty?'Button':text)); 
  } 
} 
...
class _SelectWrapperState extends State<_SelectWrapper> { 
  ...
  @override 
  Widget build(BuildContext context) { 
    return DropdownButtonFormField<String>(initialValue: widget.items.any((e) => e.value == _val) ? _val : null, decoration: const InputDecoration(border: OutlineInputBorder(), contentPadding: EdgeInsets.symmetric(horizontal: 8, vertical: 8)), items: widget.items.map((i) => DropdownMenuItem(value: i.value, child: Text(i.key))).toList(), onChanged: (v) { 
      setState(() => _val = v); 
      if(widget.formData!=null&&widget.name.isNotEmpty) 
...
class _TextareaWrapperState extends State<_TextareaWrapper> { 
  ...
  @override 
  Widget build(BuildContext context) => TextField(controller: _c, maxLines: 4, decoration: InputDecoration(hintText: widget.placeholder, border: const OutlineInputBorder())); 
}
